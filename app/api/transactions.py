...
async def create_transfer(
    ...
    try:
        ...
        transaction = await transaction_service.create_internal_transfer(
            transaction_data=transaction_data, org_id=organization.id
        )
...
async def list_transactions(
    ...
    try:
        ...
        transactions, total = await transaction_service.get_transactions(
            org_id=organization.id, filters=filters
        )

        return TransactionListResponse(
            transactions=transactions, total_count=total, page=page, page_size=page_size
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_transaction(
    ...
    try:
        ...
        transaction = await transaction_service.get_transaction_by_id(
            transaction_id=uuid.UUID(transaction_id), org_id=organization.id
        )
...
async def get_transaction_summary(
    ...
    try:
        ...
        for txn in transactions:
            ...
            type_counts[txn.transaction_type.value] = (
                type_counts.get(txn.transaction_type.value, 0) + 1
            )
